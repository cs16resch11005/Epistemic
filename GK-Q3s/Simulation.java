...
public class Simulation 
{
	...
	protected static Integer [] posStates1;
	...
	Simulation(int runId)
	{
		...
		posStates1 = new Integer[ConfigParameters.num_People];
	}
	...
	public void copyPosStates()
	{
		for(int j=0; j<ConfigParameters.num_People; j++)
			posStates1[j] = population.get(j).currState;
	}
	...
	public void Run() 
	{
		for(int i=0; i<ConfigParameters.sim_Time; i++)
		{
			...
			isQuerySatisfied = checkCommonKnowledgeOperator();
	...
	public boolean checkCommonKnowledgeOperator()
	{
		...
		copyPosStates();
	 	...
	 	for(int i=0; i<4; i++)
	 	{
			EpistemicRelatedStates ers1 = new EpistemicRelatedStates();
	 		isvalid =  ers1.generateEpistemicPossibleStates(i, posStates1);
	 		if(isvalid == false)
	 			return false;
	 	}
